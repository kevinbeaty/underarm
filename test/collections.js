...
test('find', function(t) {
  ...
  _r(array).async().find(function() { return false })
    .then(function(result){
      t.deepEqual(result, void 0, 'should return `undefined` if `value` is not found')
    })
  ...
  _r(list)
    .find({c: 1})
    .then(function(result){
      t.ok(result === void 0, 'undefined when not found')
    })

  _r(resolve([]))
    .find({c: 1})
    .then(function(result){
      t.ok(result === void 0, 'undefined when searching empty list')
    })
